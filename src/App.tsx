...
const C = {
  ...
  borderLight: '#2a2a2a',
  yellow: '#F5C518',
  yellowBg: 'rgba(245,197,24,0.08)',
  yellowBgHover: 'rgba(245,197,24,0.14)',
  white: '#FFFFFF',
  gray: '#888888',
  grayLight: '#555555',
  success: '#22c55e',
...
const STATUS_COLORS = {
  'Aguardando pagamento': [C.warn, C.warnBg],
  Pago: [C.info, C.infoBg],
  Separado: [C.yellow, C.yellowBg],
  Enviado: [C.warn, C.warnBg],
  Entregue: [C.success, C.successBg],
  Finalizado: [C.success, C.successBg],
  Cancelado: [C.danger, C.dangerBg],
  Pendente: [C.warn, C.warnBg],
  Atrasado: [C.danger, C.dangerBg],
  'Em trânsito': [C.info, C.infoBg],
  Recebido: [C.success, C.successBg],
  'Aguardando envio': [C.info, C.infoBg],
  'Saiu para entrega': [C.warn, C.warnBg],
  'Pedido realizado': [C.info, C.infoBg],
  Publicado: [C.info, C.infoBg],
  Concluído: [C.success, C.successBg],
  PIX: [C.success, C.successBg],
  Crédito: [C.info, C.infoBg],
  Débito: [C.info, C.infoBg],
  Dinheiro: [C.yellow, C.yellowBg],
  Fiado: [C.danger, C.dangerBg],
  Torcedor: [C.info, C.infoBg],
  Jogador: [C.yellow, C.yellowBg],
  Retrô: [C.warn, C.warnBg],
  Polo: [C.success, C.successBg],
  Treino: [C.gray, '#333'],
};
...
function KpiCard({ label, value, sub, accent, icon }) {
  return (
    ...
    >
      ...
      <div style={{ ...S.bigNumber, color: accent || C.white, fontSize: 26 }}>
        {value}
      </div>
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr>
              <td
                colSpan={cols.length}
                style={{ padding: 32, textAlign: 'center', color: C.gray }}
              >
                {emptyMsg}
              </td>
            </tr>
...
function Dashboard({ onNav }) {
  ...
  const aReceber = fiado
    .filter((f) => f.status !== 'Pago')
    .reduce((a, f) => a + Number(f.valor || 0), 0);
  const valorEstoque = produtos.reduce(
    (a, p) => a + Number(p.custo || 0) * Number(p.quantidade || 0),
    0
  );
  const baixoEstoque = produtos.filter(
    (p) => Number(p.quantidade) > 0 && Number(p.quantidade) <= 3
  );
  ...
  return (
    <div>
      ...
      {vendas.length === 0 && (
        ...
        >
          ⚡ <strong>Bem-vindo ao Vibra FC ERP!</strong> Comece cadastrando
          produtos e registrando suas primeiras vendas.
          ...
          zerado.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {atrasados.map((f) => (
                ...
                >
                  🚨 Fiado atrasado: <strong>{f.cliente}</strong> —{' '}
                  {fmt(f.valor)}
                </div>
              ))}
              {zerado.map((p) => (
              ...
              {baixoEstoque.map((p) => (
                ...
                >
                  ⚠️ Estoque baixo: <strong>{p.nome}</strong> — {p.quantidade}{' '}
                  un.
                </div>
              ))}
            </div>
...
function Produtos() {
  ...
  const filtrados = rows.filter(
    (p) =>
      ...
      p.clube?.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const salvar = async () => {
    ...
    try {
      ...
      if (editando) {
        await edit(editando.id, payload);
      } else {
        await add(payload);
      }
      setModal(false);
  ...
  const excluir = async (id) => {
    if (confirm('Excluir produto?')) await remove(id);
  };
  ...
  return (
    <div>
      ...
      <SectionHeader
        ...
        action={<Btn onClick={() => abrir()}>+ Novo Produto</Btn>}
      />
      <div style={{ marginBottom: 16 }}>
        <Input
          placeholder="Buscar produto ou clube..."
          value={search}
        ...
        >
          ...
          >
            <div style={{ gridColumn: '1/-1' }}>
              <Field label="Nome">
                <Input
                  ...
                  placeholder="Ex: Brasil 2026 Home"
                />
              </Field>
            </div>
            ...
            <Field label="Temporada">
              <Input
                ...
                onChange={(e) =>
                  setForm({ ...form, temporada: e.target.value })
                }
              />
            </Field>
            <Field label="Categoria">
              <Select
                value={form.categoria || 'Torcedor'}
                onChange={(e) =>
                  setForm({ ...form, categoria: e.target.value })
                }
                options={['Torcedor', 'Jogador', 'Retrô', 'Polo', 'Treino']}
              />
            </Field>
            <Field label="Cor">
              <Input
          ...
          {form.custo && form.preco && (
            ...
            >
              Margem:{' '}
              {Math.round(
                ((Number(form.preco) - Number(form.custo)) /
                  Number(form.preco)) *
                  100
              )}
              % · Lucro unitário: {fmt(Number(form.preco) - Number(form.custo))}
            </div>
          )}
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <Btn ghost onClick={() => setModal(false)}>
              Cancelar
            </Btn>
          </div>
        </Modal>
  ...
  return (
    <div>
      ...
      <SectionHeader
        ...
        action={<Btn onClick={() => setModal(true)}>+ Nova Venda</Btn>}
      />
      <div style={S.card}>
        <Table
          cols={[
      ...
      {modal && (
        <Modal title="Registrar Venda" onClose={() => setModal(false)}>
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                setForm({
                  ...
                  custo: p?.custo || form.custo,
                  tamanho: p?.tamanho || form.tamanho,
                  valor: p?.preco || form.valor,
                });
              }}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <Btn onClick={salvar} disabled={saving}>
              {saving ? 'Salvando...' : 'Concluir Venda'}
            </Btn>
            <Btn ghost onClick={() => setModal(false)}>
              Cancelar
            </Btn>
...
function Clientes() {
  ...
  const msgWhats = (nome, valor) =>
    encodeURIComponent(
      `Fala, ${nome}! Passando para lembrar do pagamento de ${fmt(
        valor
      )} da Vibra FC. Pode fazer o PIX e me manda o comprovante. Obrigado! ⚡`
    );
  ...
  const excluir = async (id) => {
    if (confirm('Excluir cliente?')) await remove(id);
  };
  ...
  return (
    <div>
      ...
      >
        ...
        <KpiCard
          label="Com compras"
          value={[...new Set(vendas.map((v) => v.cliente_nome))].length}
          icon="🛍️"
        />
        <KpiCard
          label="Faturamento Total"
          value={fmt(vendas.reduce((a, v) => a + Number(v.valor || 0), 0))}
          icon="💰"
        />
      </div>
      <SectionHeader
        title="CRM de Clientes"
        action={<Btn onClick={() => setModal(true)}>+ Novo Cliente</Btn>}
      />
      <div style={S.card}>
        <Table
          cols={[
            ...
            { key: 'time_favorito', label: 'Time' },
            {
              ...
              label: 'Compras',
              align: 'center',
              render: (v) => clienteVendas(v).length,
            },
            {
              key: 'nome',
              label: 'Total Gasto',
          ...
          {(() => {
            ...
            return (
              <>
                ...
                >
                  {detalhe.whatsapp && (
                    <div>
                      ...
                      <span style={{ color: C.text }}>{detalhe.whatsapp}</span>
                    </div>
                  )}
                  {detalhe.instagram && (
                    <div>
                      <span style={S.label}>Instagram: </span>
                      <span style={{ color: C.text }}>{detalhe.instagram}</span>
                    </div>
                  )}
                  {detalhe.cidade && (
                    <div>
                      <span style={S.label}>Cidade: </span>
                      <span style={{ color: C.text }}>{detalhe.cidade}</span>
                    </div>
                  )}
                  {detalhe.time_favorito && (
                    <div>
                ...
                {detalhe.whatsapp && (
                  <a
                    href={`https://wa.me/55${detalhe.whatsapp}?text=${msgWhats(
                      detalhe.nome,
                      total
                    )}`}
                    target="_blank"
      ...
      {modal && (
        <Modal title="Novo Cliente" onClose={() => setModal(false)}>
          ...
          >
            ...
            <Field label="Time Favorito">
              <Input
                value={form.time_favorito}
                onChange={(e) =>
                  setForm({ ...form, time_favorito: e.target.value })
                }
              />
            </Field>
            <div style={{ gridColumn: '1/-1' }}>
              <Field label="Observações">
                <Input
                  value={form.obs}
                  onChange={(e) => setForm({ ...form, obs: e.target.value })}
                  placeholder="Preferências, indicações, observações..."
                />
              </Field>
            </div>
          </div>
...
function Pedidos() {
  ...
  const moverStatus = async (id, status) => {
  ...
  return (
    <div>
      <SectionHeader
        ...
        action={<Btn onClick={() => setModal(true)}>+ Novo Pedido</Btn>}
      />
                ...
                >
                  {cards.map((card) => (
                    ...
                    >
                      ...
                      <div
                        style={{ fontSize: 11, color: C.gray, marginBottom: 4 }}
                      >
                        {card.produto} — {card.tamanho}
                      </div>
                      <div
                        style={{
                          fontSize: 12,
                      ...
                      >
                        {KANBAN_COLS.filter((k) => k.key !== card.status)
                          .slice(0, 2)
                          .map((k) => (
                            <button
                              key={k.key}
                              onClick={() => moverStatus(card.id, k.key)}
                              style={{
                                fontSize: 10,
                                padding: '2px 7px',
...
function Fiado() {
  ...
  const marcarPago = async (id) => {
    await edit(id, { status: 'Pago' });
  };
  const msg = (nome, valor) =>
    encodeURIComponent(
      `Fala, ${nome}, tudo certo? Passando para lembrar do pagamento de ${fmt(
        valor
      )} da Vibra FC. Assim que fizer o PIX, me manda o comprovante. Obrigado! ⚡`
    );
  ...
  return (
    <div>
      ...
      {atrasados.length > 0 && (
        ...
        >
          <div style={{ ...S.sectionTitle, color: C.danger }}>
          ...
          {atrasados.map((f) => (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <a
                  href={`https://wa.me/55${f.whatsapp}?text=${msg(
                    f.cliente,
                    f.valor
                  )}`}
                  target="_blank"
                  rel="noreferrer"
                >
                  <Btn style={{ ...S.btn, padding: '6px 12px', fontSize: 11 }}>
                    💬 Cobrar
                  </Btn>
                </a>
              </div>
      ...
      <SectionHeader
        ...
        action={<Btn onClick={() => setModal(true)}>+ Registrar Fiado</Btn>}
      />
      <div style={S.card}>
        <Table
          cols={[
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  {r.status !== 'Pago' && (
                    <>
                      ...
                      {r.whatsapp && (
                        <a
                          href={`https://wa.me/55${r.whatsapp}?text=${msg(
                            r.cliente,
                            r.valor
                          )}`}
                          target="_blank"
    ...
    <div>
      ...
      >
        ...
        <KpiCard
          ...
          accent={entradas - saidas >= 0 ? C.yellow : C.danger}
          icon="💵"
        />
      </div>
      <SectionHeader
            ...
            {['todos', 'entrada', 'saida'].map((f) => (
              <button
                ...
                style={{
                  ...S.btnGhost,
                  padding: '6px 14px',
                  background: filtro === f ? C.yellowBg : 'transparent',
                  color: filtro === f ? C.yellow : C.gray,
                  borderColor: filtro === f ? C.yellow : C.border,
                }}
              >
                {f === 'todos'
                  ? 'Todos'
          ...
          cols={[
            { key: 'data', label: 'Data' },
            {
              ...
              render: (v) => (
                <span
                  style={{
                    color: v === 'entrada' ? C.success : C.danger,
                    fontWeight: 700,
                    fontSize: 11,
                    textTransform: 'uppercase',
                  }}
            ...
            { key: 'descricao', label: 'Descrição' },
            {
              ...
              render: (v, r) => (
                <span
                  style={{
                    color: r.tipo === 'entrada' ? C.success : C.danger,
                    fontWeight: 700,
                  }}
                >
                  {r.tipo === 'saida' ? '−' : '+'}
...
function Metas() {
  ...
  const getAtual = (m) => {
    ...
    if (m.tipo === 'Ticket médio')
      return vendas.length > 0 ? fatTotal / vendas.length : 0;
  ...
  return (
    <div>
      <SectionHeader
        title="Metas"
        action={<Btn onClick={() => setModal(true)}>+ Nova Meta</Btn>}
      />
      {rows.length === 0 ? (
        <div
          style={{ ...S.card, textAlign: 'center', padding: 40, color: C.gray }}
        >
          Nenhuma meta criada ainda. Defina sua primeira meta!
        </div>
      ) : (
        <div
          style={{
            ...
            return (
              <div key={m.id} style={S.card}>
                ...
                >
                  ...
                  >
                    <span style={{ ...S.badge(cor, cor + '22'), fontSize: 13 }}>
                      {p}%
                    </span>
                    <button
                      onClick={() => remove(m.id)}
                ...
                >
                  <span style={{ fontSize: 26, fontWeight: 800, color: cor }}>
                    {m.unidade === 'R$' ? fmt(atual) : Math.round(atual)}
                  </span>
                ...
                {p >= 100 ? (
                  ...
                  <div style={{ marginTop: 10, fontSize: 12, color: C.gray }}>
                    ...
                    <span style={{ color: C.text }}>
                      {m.unidade === 'R$'
                        ? fmt(Number(m.meta) - atual)
                        : `${Math.round(Number(m.meta) - atual)} ${m.unidade}`}
                    </span>
                  </div>
                )}
              </div>
...
function Fornecedores() {
  const {
    ...
    loading,
    add: addForn,
  } = useTable('fornecedores', 'nome.asc');
  const {
    rows: compras,
    add: addCompra,
    edit: editCompra,
  ...
  const salvarCompra = async () => {
    ...
    setSaving(true);
    try {
      const qtd = Number(formC.quantidade),
        vunit = Number(formC.valor_unit),
        frete = Number(formC.frete || 0);
      await addCompra({
        ...formC,
        quantidade: qtd,
        valor_unit: vunit,
        frete,
        total: qtd * vunit + frete,
      });
  ...
  return (
    <div>
      ...
      >
        ...
        <KpiCard
          label="Em Trânsito"
          value={compras.filter((c) => c.status === 'Em trânsito').length}
          accent={C.warn}
          icon="🚚"
        />
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              fontSize: 13,
              fontWeight: 700,
              background: aba === a.key ? C.yellow : C.surface,
              color: aba === a.key ? '#000' : C.gray,
            }}
          >
            {a.label}
          </button>
      ...
      {aba === 'fornecedores' && (
        <>
          <SectionHeader
            title="Fornecedores"
            action={
              <Btn onClick={() => setModalForn(true)}>+ Novo Fornecedor</Btn>
            }
          />
            ...
            >
              {fornecedores.map((f) => (
                <div key={f.id} style={S.card}>
                  ...
                  {f.whatsapp && (
                    <a
                      href={`https://wa.me/55${f.whatsapp}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      <Btn
                        style={{ ...S.btn, padding: '6px 14px', fontSize: 11 }}
                      >
                        💬 WhatsApp
                      </Btn>
                    </a>
      ...
      {aba === 'compras' && (
        <>
          <SectionHeader
            ...
            action={
              <Btn onClick={() => setModalCompra(true)}>+ Registrar Compra</Btn>
            }
          />
          <div style={S.card}>
            <Table
                ...
                },
                {
                  ...
                  render: (v, r) => (
                    <select
                      value={r.status}
                      onChange={(e) =>
                        editCompra(v, { status: e.target.value })
                      }
                      style={{
                        ...S.input,
                        padding: '4px 8px',
          ...
          >
            <div style={{ gridColumn: '1/-1' }}>
              <Field label="Nome">
                <Input
                  value={formF.nome}
                  onChange={(e) => setFormF({ ...formF, nome: e.target.value })}
                />
              </Field>
            </div>
            <Field label="WhatsApp">
              <Input
                value={formF.whatsapp}
                onChange={(e) =>
                  setFormF({ ...formF, whatsapp: e.target.value })
                }
              />
            </Field>
            <Field label="Cidade">
              <Input
                value={formF.cidade}
                onChange={(e) => setFormF({ ...formF, cidade: e.target.value })}
              />
            </Field>
            <Field label="Prazo médio">
              <Input
                value={formF.prazo}
                onChange={(e) => setFormF({ ...formF, prazo: e.target.value })}
            ...
            <div style={{ gridColumn: '1/-1' }}>
              <Field label="Observações">
                <Input
                  value={formF.obs}
                  onChange={(e) => setFormF({ ...formF, obs: e.target.value })}
                />
              </Field>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
            <Btn onClick={salvarForn} disabled={saving}>
              {saving ? 'Salvando...' : 'Salvar'}
            </Btn>
            <Btn ghost onClick={() => setModalForn(false)}>
              Cancelar
            </Btn>
          </div>
        </Modal>
      ...
      {modalCompra && (
        <Modal title="Registrar Compra" onClose={() => setModalCompra(false)}>
          <Field label="Fornecedor">
            <Select
              value={formC.fornecedor}
              onChange={(e) =>
                setFormC({ ...formC, fornecedor: e.target.value })
              }
              options={[
                { value: '', label: 'Selecione...' },
          ...
          <Field label="Produto">
            <Input
              value={formC.produto}
              onChange={(e) => setFormC({ ...formC, produto: e.target.value })}
            />
          </Field>
          <div
            style={{
          ...
          >
            <Field label="Qtd">
              <Input
                type="number"
                value={formC.quantidade}
                onChange={(e) =>
                  setFormC({ ...formC, quantidade: e.target.value })
                }
              />
            </Field>
            <Field label="Valor unit. (R$)">
              <Input
                type="number"
                value={formC.valor_unit}
                onChange={(e) =>
                  setFormC({ ...formC, valor_unit: e.target.value })
                }
              />
            </Field>
            <Field label="Frete (R$)">
              <Input
                type="number"
                value={formC.frete}
                onChange={(e) => setFormC({ ...formC, frete: e.target.value })}
              />
            </Field>
          </div>
          {formC.quantidade && formC.valor_unit && (
            ...
            >
              Total:{' '}
              {fmt(
                Number(formC.quantidade) * Number(formC.valor_unit) +
                  Number(formC.frete || 0)
              )}
            </div>
          )}
          <div
          ...
          >
            <Field label="Data">
              <Input
                type="date"
                value={formC.data}
                onChange={(e) => setFormC({ ...formC, data: e.target.value })}
              />
            </Field>
            <Field label="Prazo entrega">
              <Input
                type="date"
                value={formC.prazo_entrega}
                onChange={(e) =>
                  setFormC({ ...formC, prazo_entrega: e.target.value })
                }
              />
            </Field>
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
            <Btn onClick={salvarCompra} disabled={saving}>
              {saving ? 'Salvando...' : 'Registrar'}
            </Btn>
            <Btn ghost onClick={() => setModalCompra(false)}>
              Cancelar
            </Btn>
          </div>
        </Modal>
  ...
  return (
    <div>
      ...
      >
        <KpiCard
          label="Total Gasto"
          value={fmt(rows.reduce((a, e) => a + Number(e.valor || 0), 0))}
          icon="🚚"
        />
        <KpiCard
          label="Em Rota"
          value={rows.filter((e) => e.status === 'Saiu para entrega').length}
          accent={C.warn}
          icon="📍"
        />
        <KpiCard
          label="Entregues"
          value={rows.filter((e) => e.status === 'Entregue').length}
          accent={C.success}
          icon="✅"
        />
      </div>
              ...
              <div
                ...
                style={{
                  ...
                  borderLeft: `3px solid ${
                    e.status === 'Saiu para entrega' ? C.warn : C.info
                  }`,
                }}
              >
                <div
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {e.status === 'Aguardando envio' && (
                      <button
                        onClick={() =>
                          edit(e.id, { status: 'Saiu para entrega' })
                        }
                    ...
                    {e.status === 'Saiu para entrega' && (
                      <button
                        onClick={() => edit(e.id, { status: 'Entregue' })}
                        style={{ ...S.btn, padding: '5px 12px', fontSize: 11 }}
                      >
                        ✅ Entregue
                      </button>
      ...
      <SectionHeader
        ...
        action={<Btn onClick={() => setModal(true)}>+ Nova Entrega</Btn>}
      />
      <div style={S.card}>
        <Table
          cols={[
      ...
      {modal && (
        <Modal title="Nova Entrega" onClose={() => setModal(false)}>
          ...
          <div
            style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}
          >
            <Field label="Motoboy / Serviço">
              <Input
                value={form.motoboy}
                onChange={(e) => setForm({ ...form, motoboy: e.target.value })}
              />
            </Field>
            <Field label="Valor (R$)">
              <Input
            ...
            <Field label="Status">
              <Select
                ...
                options={['Aguardando envio', 'Saiu para entrega', 'Entregue']}
              />
            </Field>
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
...
function DinheiroPedro() {
  ...
  const salvar = async () => {
    ...
    try {
      await add({ ...form, valor: Number(form.valor) });
      setModal(false);
      setForm({ valor: '', data: today(), motivo: '' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <div>
            <div style={S.label}>Lucro da Empresa</div>
            <div
              style={{
                ...S.bigNumber,
                fontSize: 22,
          ...
          <div>
            <div style={S.label}>Total Retirado</div>
            <div
              style={{
                ...S.bigNumber,
                fontSize: 22,
          ...
          <div>
            <div style={S.label}>Disponível p/ Retirada</div>
            <div
              style={{
                ...S.bigNumber,
                fontSize: 22,
        ...
        <div style={S.card}>
          ...
          {retiradas.length === 0 ? (
            ...
            retiradas.map((r) => (
              ...
              >
                <div>
                  <div style={{ fontSize: 13, color: C.text }}>{r.motivo}</div>
                  <div style={{ fontSize: 11, color: C.gray }}>{r.data}</div>
                </div>
                <span style={{ color: C.danger, fontWeight: 700 }}>
                  {fmt(r.valor)}
                </span>
  ...
  return (
    ...
    >
      ...
      >
          ...
          {MODULOS.map((m) => (
            <button
              ...
              style={{
                ...
                padding: '5px 10px',
                borderRadius: 6,
                background: ativo === m.id ? C.yellowBg : 'transparent',
                color: ativo === m.id ? C.yellow : C.gray,
                border:
                  ativo === m.id
                    ? `1px solid ${C.yellow}33`
                    : '1px solid transparent',
                cursor: 'pointer',
                fontSize: 11,
                fontWeight: ativo === m.id ? 700 : 500,
                whiteSpace: 'nowrap',
              }}
            >
              <span>{m.icon}</span>
        ...
        <div style={{ fontSize: 10, color: C.grayLight, whiteSpace: 'nowrap' }}>
          🟢 Conectado
        </div>
      </div>

      <div
        style={{ maxWidth: 1280, margin: '0 auto', padding: '24px 16px 60px' }}
      >
        <div
          style={{
            display: 'flex',
        ...
        >
          ...
          <span style={{ fontSize: 11, color: C.grayLight, marginLeft: 4 }}>
            · Vibra FC ERP
          </span>
        </div>
